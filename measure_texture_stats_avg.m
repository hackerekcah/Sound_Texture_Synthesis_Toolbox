...
%
% modified March 2017 to replace wavwrite with audiowrite
...
for f=1:n_files
    ...
    if P.write_norm_orig
        if length(orig_sound)>1.5*length(P.desired_synth_dur_s)
            audiowrite([P.output_folder 'norm_' P.files_for_stat_avg{f}], orig_sound(1:P.desired_synth_dur_s*P.audio_sr), P.audio_sr);
        else
            audiowrite([P.output_folder 'norm_' P.files_for_stat_avg{f}], orig_sound, P.audio_sr);
        end
    end
